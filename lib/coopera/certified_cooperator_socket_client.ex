defmodule Coopera.CertifiedCooperatorSocketClient do
  @moduledoc false
  require Logger
  ...
  def start_link() do
    IO.puts("CertifiedCoopeartor.start_link()")
  ...
  def init(url) do
    IO.puts("CertifiedCoopeartor.init()")
    {:connect, url, [], %{first_join: true, ping_ref: 1}}
  end

  def handle_connected(transport, state) do
    Logger.info("connected")
    GenSocketClient.join(transport, "certified:cooperator")
    {:ok, state}
  end
  ...
  def handle_info(:ping_server, transport, state) do
    Logger.info("cert sending ping ##{state.ping_ref}")
    #GenSocketClient.push(transport, "certified:cooperator", "ping", %{ping_ref: state.ping_ref})
